...
the three group-specific belief measures which are are collected separately for Ukrainian and Syrian migrants,
\[
B_{ig}
\in
\left\{
...
with their beliefs about migrants' integration outcomes and may thereby provide additional information for 
interpreting the mechanisms underlying these beliefs.
...
\end{equation}
The coefficient \(\lambda_{LM}^{k}\) captures the conditional association between stronger beliefs that migrants' 
...
+
\lambda_{Soc}TimeSoc_i
+
\boldsymbol{\delta}^{\mathrm{Soc}\prime}X_i
+
\varepsilon_{ij}^{\mathrm{Soc}}.
\end{equation}
Here, \(\lambda_{Soc}\) captures the conditional association between stronger beliefs that migrants become more socially 
...
\noindent\textbf{Statistical inference and multiple testing.}
Since each respondent evaluates several vignette profiles, standard errors are
clustered at the respondent level throughout the analysis. Estimated
coefficients are reported together with 95\% confidence intervals. Several of
the hypotheses are examined across more than one outcome, which creates a
multiple-testing concern. To account for this, the main hypothesis tests are
additionally evaluated using Holm-adjusted $p$-values within the respective
pre-specified hypothesis families. Unadjusted estimates and confidence
intervals are reported alongside the adjusted inference so that the direction,
magnitude, and uncertainty of the estimated relationships remain transparent.

\noindent\textbf{Robustness.}
The main experimental specifications are estimated using linear regression,
with the specifications including respondent-level controls serving as an
adjusted robustness analysis. For the ordinal income, discrimination, and
social-integration outcomes, ordered logit models are additionally estimated
to examine whether the conclusions are sensitive to treating the ordered
response categories as approximately equally spaced in the linear
specifications. For the associational analyses in Sections 4.2 and 4.3,
robustness analyses may additionally use respondent fixed effects where the
relevant variables vary within respondents across the Ukrainian and Syrian
groups. These specifications account for respondent characteristics that are
constant across the two groups, although they do not eliminate potential
confounding arising from group-specific experiences or beliefs.

\noindent\textbf{Data treatment.}
Data-quality procedures follow the approach described in Section 3.5.
Responses are assessed using the pre-specified attention check, survey
completion times, and available indicators of duplicate or automated
responding. The precise thresholds used to flag potentially low-quality
responses are determined following the pilot study and prior to the main
analysis. The main results are additionally compared with results obtained
when respondents flagged for potential low-quality responding are retained.

Missing and not-applicable responses are distinguished from indicators of
low-quality responding and are treated according to the construction of the
respective variable. In particular, measures that are only observed following
a preceding response, such as the valence of a memorable personal experience
or memorable media information, are analyzed only when the corresponding
measure is observed rather than assigning an artificial value to structurally
missing responses. The construction and treatment of derived variables and
missing responses are documented consistently before the main analysis.

\noindent\textbf{Interpretation and limitations.} The interpretation of the results reflects the distinction between the experimental and associational parts of the empirical strategy. Random assignment of the vignette characteristics allows their estimated coefficients to be interpreted as causal effects of the presented profile characteristics on respondents' stated beliefs. By contrast, respondents' contact, experiences, media exposure, memorable information, and broader integration beliefs are not randomly assigned. Relationships involving these measures are therefore interpreted as conditional associations rather than causal effects. Moreover, some of these measures may be related to one another, such as overall and memorable experience valence, overall and memorable media valence, or the different community-belief measures. Their coefficients are therefore not interpreted as identifying independent causal mechanisms, but as providing complementary information that may help interpret patterns in respondents' beliefs. The corresponding limitations and possible extensions are discussed in Section 5.
